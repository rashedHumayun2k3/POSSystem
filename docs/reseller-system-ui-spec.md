...
- **List** (reached from Settings): two sections **Managing Partners** / **Sleeping Partners**, each a [cards] list — name · phone · capital balance (৳, paisa÷100) · deferred-loss badge if >0 · status chip (ACTIVE/EXITED). [fab] "+ New Partner".
- **New Partner:** [input name] · [input phone] · [seg MANAGING/SLEEPING] · [input join date] · [input note] · [btn Save]. Type field locks (greyed, "cannot change after first capital entry") once any ledger row exists — shown on edit, not on create.
- **Partner Detail:** header card (name, type chip, status) → **Balance row:** Capital balance · Profit balance (15b+, shows ৳0.00 for now) · Deferred loss (red badge if >0) → [btn "+ Add Capital Injection"] (owner only) → **Ledger timeline [cards]**, newest first: date · entry type chip · signed amount (green +/red −) · balance-after · note; read-only, no edit/delete affordance anywhere (matches R15.3 immutability).
