...
export default function FlashSaleSection() {
  ...
  // Own query key (sort: "discount") — the backend now does the discount-threshold filter and
  // ordering itself, so this no longer shares a fetch with the plain browse/search grid.
  const { data, isLoading, hasNextPage, isFetchingNextPage, fetchNextPage } = useInfiniteQuery({
    queryKey: ["clientpage-products", shopSlug, "discount"],
    queryFn: ({ pageParam }) => searchProducts(shopSlug, { sort: "discount", page: pageParam, pageSize: PAGE_SIZE }),
    initialPageParam: 1,
