...
import { loadExportedStorefrontConfig } from "./storefrontConfig";
...
api.interceptors.request.use(async (config) => {
  const storefrontConfig = await loadExportedStorefrontConfig();
  if (storefrontConfig?.apiBaseUrl) {
    config.baseURL = storefrontConfig.apiBaseUrl.replace(/\/$/, "");
  }
...
mediaApi.interceptors.request.use(async (config) => {
  const storefrontConfig = await loadExportedStorefrontConfig();
  if (storefrontConfig?.mediaBaseUrl) {
    config.baseURL = storefrontConfig.mediaBaseUrl.replace(/\/$/, "");
  }
