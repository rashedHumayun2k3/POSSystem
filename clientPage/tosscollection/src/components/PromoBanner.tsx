...
const SLIDES = [
  {
    ...
    subtitle: "Engineered for speed, comfort, and confidence—every step of the way",
  ...
  },
  {
    ...
    subtitle: "Flexible comfort and effortless style for movement, balance, and everyday performance",
  ...
  },
  {
    ...
    subtitle: "Technical gear designed for the road, the water, and every challenge ahead",
    gradient: "from-amber-500 via-orange-500 to-rose-500",
    icon: "🚚",
    image: "/banners/slide-3.jpg",
  },
  {
    title: "Stronger With Every Rep",
    subtitle: "Performance wear built to move, stretch, and power through your toughest sessions",
...
export default function PromoBanner() {
  ...
  // TossCollection uses local static banners instead of database banner settings.
  const shopSlides = TOSS_STATIC_SLIDES;
  ...
  if (mode === "shop" && shopSlides.length > 0 && !bannerFailed) {
    return (
      <div className="relative w-full aspect-[16/7] bg-gray-100 overflow-hidden">
        <div
          key={shopSlides[active] ?? shopSlides[0]}
          className="absolute inset-0 grid grid-cols-5"
          role="img"
          aria-label={shopName ?? "Shop banner"}
        >
          {Array.from({ length: 5 }, (_, index) => (
            <div
              key={index}
              className="toss-slider-slice h-full bg-cover bg-no-repeat"
              style={{
                backgroundImage: `url(${shopSlides[active] ?? shopSlides[0]})`,
